...
import { parseAnalysisJob, parseAnalysisJobCreated } from '@/features/ai/api/parse';
import { AI_ANALYZE_TIMEOUT_MS } from '@/features/ai/constants';
import type { AiAnalysisJob, AiAnalysisJobCreated } from '@/features/ai/types';
...
export async function createAiAnalysisJob({
  slug,
  videoUri,
}: CreateAiJobInput): Promise<AiAnalysisJobCreated> {
  await assertExternalAiAcknowledged();
  const baseUrl = getApiBaseUrl();
  const url = `${baseUrl}/api/reference-techniques/${encodeURIComponent(slug)}/ai-analysis/jobs`;
  const formData = createVideoUploadForm(videoUri);
  ...
  try {
    ...
    return parseAnalysisJobCreated(body);
  } catch (error) {
...
export async function fetchAiAnalysisJob(
  jobId: string,
  signal?: AbortSignal,
): Promise<AiAnalysisJob> {
  const baseUrl = getApiBaseUrl();
  const url = `${baseUrl}/api/ai-analysis/jobs/${encodeURIComponent(jobId)}`;
  try {
    const response = await expoFetch(url, {
      method: 'GET',
      signal,
